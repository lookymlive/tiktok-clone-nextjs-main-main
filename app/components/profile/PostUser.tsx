...
// Importamos el componente Link de Next.js, que nos permite crear enlaces entre páginas de nuestra aplicación
...
export default function PostUser({ post }: PostUserCompTypes) {
    ...
    // Usamos el hook useEffect para ejecutar una función cuando el componente se monta
    useEffect(() => {
        ...
        setTimeout(() => {
            ...
            // Añadimos otro evento al elemento video para que se pause cuando el cursor sale de él
            video.addEventListener('mouseleave', () => { video.pause() })
        }, 50)

    }, []) // El array vacío indica que solo queremos ejecutar la función una vez

    // Devolvemos el JSX que define la apariencia del componente
    return (
        <>
            <div className="relative brightness-90 hover:brightness-[1.1] cursor-pointer">
                {/* Usamos un operador ternario para mostrar un contenido u otro dependiendo de si el post tiene o no una URL de video */}
                {!post.video_url ? (
